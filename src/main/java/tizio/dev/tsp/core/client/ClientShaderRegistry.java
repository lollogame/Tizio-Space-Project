...
public class ClientShaderRegistry {
    ...
    private static ShaderInstance sunFlareShader;
    private static ShaderInstance planetSurface;
    private static ShaderInstance planetRing;
    private static ShaderInstance planetRingRocks;

    private static ShaderInstance skyboxAtmosphere;
    private static ShaderInstance skyClouds;
    ...
    public static void registerShaders(RegisterShadersEvent event) throws IOException {
        ...
        addShader(event, "planet_ring_rocks", shader-> planetRingRocks = shader);
        addShader(event, "sun", shader-> sunShader = shader);
        addShader(event, "sun_flare", shader-> sunFlareShader = shader);

        addShader(event, "sky_atmosphere", shader-> skyboxAtmosphere = shader);
        addShader(event, "sky_clouds", shader-> skyClouds = shader);
    }
    ...
    private static void addShader(RegisterShadersEvent event, String shaderId, Consumer<ShaderInstance> setter) throws IOException {
        event.registerShader(
                new ShaderInstance(event.getResourceProvider(), ResourceLocation.fromNamespaceAndPath(MainClass.MODID, ("define/" + shaderId)), DefaultVertexFormat.POSITION_TEX),
                setter
        );
    }
    ...
    public static ShaderInstance sunFlareShader() { return sunFlareShader; }
    public static ShaderInstance planetSurface() { return planetSurface; }
    public static ShaderInstance planetRing() { return planetRing; }
    public static ShaderInstance planetRingRocks() { return planetRingRocks; }

    public static ShaderInstance skyboxAtmosphere() { return skyboxAtmosphere; }
    public static ShaderInstance skyClouds() { return skyClouds; }
