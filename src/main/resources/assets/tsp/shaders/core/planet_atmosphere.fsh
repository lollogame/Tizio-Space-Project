...
float integrateOpticalDepth(vec3 startPoint, vec3 stepVec, float innerRadius, float outerRadius, float falloff) {
...
vec4 calculateAtmosphereEffect(Ray viewRay, vec3 sunDirection, AtmosphereProperties props) {
    ...
    for (int i = 0; i < VIEW_SAMPLES; ++i) {
        ...
        float sunOpticalDepth = integrateOpticalDepth(currentPoint, sunStep, innerRadius, outerRadius, props.densityFalloff);
